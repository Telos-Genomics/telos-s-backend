...
def alinear_sincronizar(ref_path, var_path):
    ...
    nombre_ref_final = f"{folder_path}/{ref_path.replace('.txt', '').replace('output/s/spike/','')}.txt"
    nombre_var_final = f"{folder_path}/{var_path.replace('.txt', '').replace('output/s/spike/','')}.txt"
